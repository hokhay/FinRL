...
"""
    ...
    #     super(TensorboardCallback, self).__init__(env, **kwargs)
        # self.best_mean_reward = -np.inf
        # self.best_model_save_path = best_model_save_path

    # def _on_step(self):
        # super(TensorboardCallback, self)._on_step()
        # mean_reward = self.locals['rewards'][0]
...
class DRLAgent:
    ...
    # def train_model(self, model, tb_log_name, total_timesteps=5000, best_model_save_path=None, run_id=str(random.randint(1, 1000))):
    #     print('run ID :' + run_id)
    #     model = model.learn(total_timesteps=total_timesteps, tb_log_name=tb_log_name,
    #                         callback=TensorboardCallback(model.env,
    #                                                      best_model_save_path=best_model_save_path,
    #                                                      log_path=best_model_save_path,
    #                                                      eval_freq=80000, # Run it every 2 training iterations
    #                                                      n_eval_episodes=5, # Run 30 games
    #                                                      deterministic=True,
    #                                                      render=False)
    #                         )
        return model
...
class DRLEnsembleAgent:
    ...
    def run_ensemble_strategy(self, A2C_model_kwargs, PPO_model_kwargs, DDPG_model_kwargs, timesteps_dict):
        ...
        for i in range(self.rebalance_window + self.validation_window, len(self.unique_trade_date),
            ...
            end_date_index = self.df.index[self.df["date"] == self.unique_trade_date[
            ...
            model_a2c = self.train_model(model_a2c, "a2c", tb_log_name="a2c_{}".format(i), iter_num=i,
            ...
            model_ppo = self.train_model(model_ppo, "ppo", tb_log_name="ppo_{}".format(i), iter_num=i,
            ...
            model_ddpg = self.train_model(model_ddpg, "ddpg", tb_log_name="ddpg_{}".format(i), iter_num=i,
        ...
        df_summary = pd.DataFrame(
            [iteration_list, validation_start_date_list, validation_end_date_list, model_use, a2c_sharpe_list,
             ppo_sharpe_list, ddpg_sharpe_list]).T
