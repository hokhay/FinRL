...
class StockTradingEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            # Construct dataframe for rewards
            df_rewards = pd.DataFrame(self.rewards_memory)
            df_rewards.columns = ['account_rewards']
            df_rewards['date'] = self.date_memory[:-1]
            # Print info
            if self.episode % self.print_verbosity == 0:
                print(f"day: {self.index_day}, episode: {self.episode}")
            ...
            if (self.model_name != '') and (self.mode != ''):
                # Construct dataframe for actions
            ...
            self.state = self._update_state() # update state
            ...
            daily_return = pd.DataFrame(self.asset_memory).pct_change(1)
            sharpe = 0
            ...
            # self.reward = daily_return.iloc[-1][0] + sharpe * self.reward_scaling
            # self.reward = daily_return.iloc[-1][0]
            self.reward = (np.array([(self.reward / begin_total_asset), daily_return.iloc[-1][0], sharpe]) * self.reward_scaling).sum()
    ...
    def reset(self):
        ...
        if self.random_start:
            starting_point = random.choice(range(int(len(self.df.date.unique()) * 0.5)))
    ...
    def get_multiproc_env(self, n=10):
        e = SubprocVecEnv([lambda: self for _ in range(n)], start_method="fork")
        obs = e.reset()
        return e, obs
